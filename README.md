...
MoodleMCP panel. The plugin provisions role-scoped Moodle web-service tokens
and turns them into revocable **MCP keys** that an assistant presents to query
your site.

## Requires a MoodleMCP account (commercial service)

**This plugin does not work on its own.** It is the Moodle-side client of the
**MoodleMCP** hosted panel — a separate, commercial subscription service. You
need an account and a license there to use it. The plugin talks to that panel
over a signed HTTPS API and to no other third party.
...
Review the panel's own terms and privacy policy before enabling this in
production; permanent web-service tokens and MCP keys are delivered by email.
...
1. Copy this directory to `<moodleroot>/local/mcpconnector` (or install the zip
   ...
   upgrade.
2. In your MoodleMCP panel, connect your Moodle site and copy the **license
   key** and **panel secret** (shown once).
